...
6. **OTLP app metrics still auto-route in Honeycomb.** The `metrics` pipeline's `otlp/honeycomb` exporter sends no `x-honeycomb-dataset` header, so Honeycomb chooses the dataset for OTLP-received app metrics (they land in an auto-named dataset rather than the old `claude_code_metrics` one). To pin those, add the header to that exporter too. **Host/container metrics are already pinned:** the `hostmetrics` + `docker_stats` receivers feed a separate `metrics/host` pipeline whose `otlp/honeycomb_hostmetrics` exporter sets `x-honeycomb-dataset: livespec-host-metrics` (same key/env, the agent-activity env), so they land in the named `livespec-host-metrics` dataset. Added for Phase P-host of the `livespec` fabro-ci-image-factoring plan.
